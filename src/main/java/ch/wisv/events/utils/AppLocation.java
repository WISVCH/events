...
public enum AppLocation {
    ...
    /**
     * Constructor AppLocation creates a new AppLocation instance.
     *
     ...
     */
    AppLocation(String name, String icon, String url, SimpleGrantedAuthority authority) {
        this.name = name;
        this.icon = icon;
        this.url = url;
     ...
     * @return List<AppLocation>
     */
    public static List<AppLocation> getLocations(Collection<GrantedAuthority> authority) {
        List<AppLocation> location = new ArrayList<>();
        stream(AppLocation.values()).forEach(x -> authority.forEach(p -> {
